...
compileCompactArray elts =
  reverse $ [ OpArray $ length elts ] ++ concatMap compile elts
...
compileObjectLiteral kvMap = go . reverse $ kvMap
  where
    ...
    val (Value e)    = compile e ++ [ OpGetValue ]
...
compileCompoundAssignment lhs op e =
  compile lhs ++ [ OpDup, OpGetValue ] ++ compile e ++ [ OpGetValue ] ++
...
compileCond e1 e2 e3 =
  ...
  ifTrue (compile e2 ++ [ OpGetValue ])
         (compile e3 ++ [ OpGetValue ])

compileCall :: (Int->OpCode) -> Expr -> [Expr] -> CompiledExpr
...
ifTrue :: CompiledExpr -> CompiledExpr -> CompiledExpr
ifTrue trueBranch falseBranch = [ OpBFalse (length a + 1) ] ++ a ++ [ OpBranch (length b) ] ++ b
  where
    a = straighten trueBranch
    b = straighten falseBranch
